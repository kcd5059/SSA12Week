...
public class RegistryTests 
{
    ...
    @Test
    public void descendantsTests() {
        List<Person> desc = registry.descendants(new SocialSecurityNumber("212345672"));
        List<Person> expectedDescendants = new ArrayList<>();
        expectedDescendants.add(registry.get(new SocialSecurityNumber("412345671")));
        expectedDescendants.add(registry.get(new SocialSecurityNumber("412345670")));
        expectedDescendants.add(registry.get(new SocialSecurityNumber("312345671")));
        expectedDescendants.add(registry.get(new SocialSecurityNumber("312345670")));
        expectedDescendants.add(registry.get(new SocialSecurityNumber("412345672")));
        assertTrue(desc.containsAll(expectedDescendants));
    }
    
    @Test
    public void ancestorsTests() {
        List<Person> anc = registry.ancestors(new SocialSecurityNumber("212345670"));
        List<Person> expectedAncestors = new ArrayList<>();
        expectedAncestors.add(registry.get(new SocialSecurityNumber("112345671")));
        expectedAncestors.add(registry.get(new SocialSecurityNumber("112345672")));
        expectedAncestors.add(registry.get(new SocialSecurityNumber("012345678")));
        expectedAncestors.add(registry.get(new SocialSecurityNumber("012345679")));
        assertTrue(anc.containsAll(expectedAncestors));
    }
    
    @Test
    public void youngestDescendantsTests() {
        Person expectedYoungest = registry.get(new SocialSecurityNumber("412345672"));
        assertEquals(expectedYoungest, registry.youngestDescendant(new SocialSecurityNumber("212345672")));
    }
    
   /*@Test
    public void oldestLivingRelativeTests() {
        Person expectedOldest = registry.get(new SocialSecurityNumber(""));
        assertEquals(expectedOldest, registry.oldestLivingRelative)
    }*/
    
    public void progenitorTests() {
        Person expectedProgenitor = registry.get(new SocialSecurityNumber("012345678"));
        assertEquals(expectedProgenitor, registry.progenitor(new SocialSecurityNumber("312345671")));
        assertEquals(expectedProgenitor, registry.progenitor(new SocialSecurityNumber("312345670")));
